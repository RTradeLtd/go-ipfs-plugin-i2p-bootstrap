...
.PHONY: plugin-ipfs
plugin-ipfs:
	$(GOCC) build -o build/go-ipfs-plugin-i2p-bootstrap.so -buildmode=plugin
	chmod +x "build/go-ipfs-plugin-i2p-bootstrap.so"
...
install:
	...
	install -Dm700 build/go-ipfs-plugin-i2p-bootstrap.so "$(IPFS_PATH)/plugins/go-ipfs-plugin-i2p-bootstrap.so"
...
.PHONY: deps
deps:
	go get -u github.com/RTradeLtd/go-ipfs-plugin-i2p-bootstrap
